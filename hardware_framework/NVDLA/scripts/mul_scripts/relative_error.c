	...
	while ((read_appr = getline(&line_appr, &len_appr, fp_appr)) != -1) {
		...
		if(absolute != 0){
			...
			if(relative_error > 0 || relative_error < 0){
				fprintf(f, "exact result is %hx*%hx = %04x (%d in decimal) -- aprox result is = %04x (%d in decimal) (sign is %s)-- error is %Lf -- absolute is %d\n",ns_a, ns_b, exact_result, exact_result, ns_appr, ns_appr, sign, relative_error,absolute);
			}		
		}else{
