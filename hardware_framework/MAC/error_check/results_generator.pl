...
while(my $line = <$inf>){
  if(index($line,"weight:")>0){	
	if ($k < 300000){			
		#Inputs and outputs are not in the same line!!!
		#line 1: A0*A1 = X
		#line 2: B0*B1 = X
		#line 3: C0*C1 = A result...						
		$C = $B;
		$B = $A;
		$A = $line;
		my $data = (split (/ /, $C, 9))[2];
		...
		#Fill with zeros if data is incomplete
		...
		#convert to decimal approximate result
		my $res =  hex($result);
		my $i = 0;
		...
		while($i<(8*$input_lenght)){
	  	...
	  	if($k>1){
			...
			#print results of output files
			print $ouf $res . "\n";
			#print exact result only if working with MAC NVDLA 16-bits
			if($flag == 1){
